...
def predict(test_data_path, model_type, model_path):
    ...
    for FOLD in range(5):
        print(os.path.join(model_path, f"{model_type}_{FOLD}_label_encoders.pkl"))
        df = pd.read_csv(test_data_path)
        encoders = joblib.load(os.path.join(model_path,f"{model_type}_{FOLD}_label_enocders.pkl"))
